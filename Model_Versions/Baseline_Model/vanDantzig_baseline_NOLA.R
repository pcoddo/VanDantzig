...
V_p             = 70092059871 # Value of goods protected by dike 
...
# Flood heights (over effective height)
flood = c(seq(0,3, 0.1))
###################################################
...
pdf("Figures/optimal_dike_height_NOLA.pdf", width = 6, height = 4.5)
...
plot(X, NPV_expected_losses, type = 'l', 
     ...
     ylab = expression(bold('Expected cost [Dollars]'))
)
