...
nrand=sample(N, nexamp)
  for(i in 1:nexamp) {
    lines(years.global+year, slr.boot[nrand[i], 1:nyears.global], col=mybluealpha1, lwd=0.5)
  }
points(years.global+year, slr.global, pch = 21, bg = myred, cex = 0.5)
lines(years.global+year,predict.hind, lty = 1, lwd = 2)
abline(h = 0, lty = 2)
...
pdf("Figures/projections_constrain.pdf", width = 6, height = 4.5)
par(oma = c(0,0,0,0)+0.1, mar = c(4,5,1,1)+0.1)
plot(years.global+year, slr.global, type="n",
     xlab=("Year"),
     ylab=("Mean sea-level anomaly [mm] \nwith respect to the year 2000"),
