...
# setwd("~/Documents/Grad/SCRiM/vanDantzig/Model_Versions/Uncertainty_SLR/Sensitivity_Analysis/Sobol/SALib/Output")
# Libraries----
...
name_list1 <- list('Storm Surge' = c('p_zero_p', 'alpha_p'),
                   'Economic' = c('V_p', 'delta_prime_p', 'k_p')
                   ,'Sea Level' = c('subs_rate', 'a', 'b', 'c', 't.star', 'c.star'))

# add Parameter symbols to plot
name_symbols <- c(bquote(p[0]), expression(alpha),
                  'V', expression(delta~"'"), 'k', 
                  expression(eta),  'a', 'b', 'c', 't*', 'c*')

# Parameter descriptions
param_desc <- c("Initial flood freq.", "Flood frequency rate",
                "Value of goods", "Discount rate", "Construction cost", 
                "Subsidence rate", "SLR anomaly", "SLR rate", "Acceleration", "Abrupt SLR rate", "Abrupt SLR year")
