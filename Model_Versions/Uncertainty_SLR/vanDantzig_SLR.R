...
# Analyze for each considered Deike heightening (eq. 1 in paper)
    ...
    sea_level_rise = t(sapply(1:length(time), function(j) {
      #sea_level_global(beta.dist$a, beta.dist$b, beta.dist$c, beta.dist$c.star, (beta.dist$t.star - year), j) /1000  })) # Beta Distribution
      sea_level_global(unif.dist$a, unif.dist$b, unif.dist$c, unif.dist$c.star, (unif.dist$t.star - year), j) /1000  })) # Uniform Distribution
