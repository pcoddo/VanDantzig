...
# Set graphical limits to only show tradeoff portion of figure
df.lims <- data.frame(50000, 50000, 0.05, 200)

# Replace zeros with lower limits (1/1,000,000) for logarithmic plotting
df[df==0] <- 10^-6
...
png("pleasework.png", width = 7.5, height = 7.5, units = 'in', res = 600)
...
for(i in 1:(d-1))
{
  for(j in 1:(d-1))
  {
    ...
    if(((d+1)-j)==3)# ((i+j)==d) && ((d+1)-j)==3 )
    {

      
      plot(panel$X1, panel$X2, type = 'n', axes = FALSE, log = 'x',
           xlim = c(min(panel$X1), x.lim),
           ylim = c(min(panel$X2), y.lim))
      smoothScatter(panel$X1, panel$X2, nbin = 500, axes = FALSE, xlab = NA, ylab = NA, log = 'x',
#                     xlim = c(min(panel$X1), max(panel$X1)/2),
#                     ylim = c(min(panel$X2), max(panel$X2)/2),
...
#       x.lim = df.lims[,(d+1)-j]
      ...
      plot(panel$X1, panel$X2, type = 'n', axes = FALSE, log = 'y',
           xlim = c(min(panel$X1), x.lim),
           ylim = c(min(panel$X2), y.lim))
      smoothScatter(panel$X1, panel$X2, nbin = 500, axes = FALSE, xlab = NA, ylab = NA, log = 'y',
#                     xlim = c(min(panel$X1), max(panel$X1)/2),
#                     ylim = c(min(panel$X2), max(panel$X2)/2),
                    nrpoints = 0, add = T, colramp = mycols, transformation = function(x) x^0.2)#, yaxs = 'i', xaxs = 'i')
      box(lwd = 1.5)
      symbols(0, 0, stars = matrix(c(rep(c(2,0.75),8)), ncol = 16), bg = "green", inches = 0.15, add = TRUE)
      lines(df.base[,(d+1)-j], df.base[,i], col = "black", lwd = 3.2)
      #points(0, 0, col = myred, pch = 8, cex = 2.5, lwd = 2)
