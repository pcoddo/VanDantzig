...
df.lims <- data.frame(max(EV_p_exceed_transient.v), 1000)
...
d=ncol(df)
      ...
      points(df.mean[,2][min_ind_mean], df.mean[,1][min_ind_mean], pch = 20, col = "black", cex = 2.5)
      ...
      # Add colorbar
      plot.new()
      image.plot(zlim=c(0,1),legend.only=TRUE, labels = FALSE, horizontal=TRUE, axex=TRUE, legend.mar = 5,legend.width=3, 
                 ...
                 legend("bottomright", 
                        c("van Dantzig (1956) baseline", "Minimum expected NPV of total costs", 
                          ...
                          "Hypothetical ideal solution within considered range", "Relative density of states of the world (unitless)", "", "", "", ""),
                        lty = c(3, NA, 1, NA, NA, NA, NA, NA, NA),
                        pch = c(20, 20, NA, 8, NA, NA, NA, NA, NA),
                        lwd = c(2, NA, 2, 1.5, NA, NA, NA, NA, NA),
                        col = c("black", "black", "black", "black", NA, NA, NA, NA, NA),
                        pt.bg = c(NA, NA, NA, NA, NA, NA, NA, NA, NA),
                        pt.cex = c(1.5, 1.5, NA, 2.2, NA, NA, NA, NA, NA),
                        cex = 1.1, box.lwd = 1.5,
                        inset = c(0.05, -0.3),
                        xpd = NA, bty='n')) 
