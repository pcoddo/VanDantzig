...
# last changes:
# August 15 2011
# /Users/klaus/Desktop/wip_local/sea-level-rise/windowing_analysis/full_code_Ryan_Aug_15
##################################################
# how to run:
...
# formatted hindcast figure
pdf(file="./output/hindcast_format.pdf")  
plot(years.global+2011,slr.global,type="p",col="green",pch=1,cex=1.,
     xlab=("Year"),
     ylab=("mean sea-level anomaly (mm) with respect to the year 2000"),
     xlim=c(1800,2010),
     ylim=c(-350,100))
...
nrand=sample(N/2,nexamp)  # decrease sampling due to cutting off outliers
  for(i in 1:nexamp) {
    lines(years.extra+2011, scenario.filter[nrand[i],], col="grey", lwd=2)
  }
points(years.global+2011,slr.global,type="p",col="green",pch=19,cex=1)
lines(years.extra+2011,predict.global,col="black",lty="solid",lwd=3)
...
legend(x="topleft",
       legend=c("Observations (Jevrejava et al (2006)",
        'polynomial best fit projection',
         'sample realizations of observed variability'),
       col=c("green","black","grey"),
       lty=c("solid","solid","solid")
       )
dev.off()
...
# use the uniform pdf
xt=seq(lb.2100,ub.2100,length=n.bin)
uniform.fit.CA=dunif(xt, min=lb.2100, max=ub.2100, log = FALSE)

#unif_wt=uniform.fit.CA*(xt[2]-xt[1])
unif_wt=uniform.fit.CA/sum(uniform.fit.CA)
print(sum(unif_wt))
...
for (n in 2:x.bin)
  {
   ...
   sample.size_tmp=unif_wt[n]*nrows
...
pdf(file="./output/fitted_uniform.pdf",height=10, width=6) 
par(mfcol=c(2,1))

# panel A: plot the beta distribution fit to Pfeffer

plot(xt,unif_wt,type="l",col="black",
     lwd=4,xlim=c(-50,3000),
     ylim=c(0,0.05),
     xlab="Projected Sea-Level Rise in 2100 [mm]",
     ylab="Probability density function", main="(a) Uniform Distribution")
# add the zero line
 lines(xt,unif_wt*0,lwd=1,lty=2,col="gray")
...
plot(xt,uniform.fit.CA,type="l",col="black", lwd=4,
     xlim=c(-50,3000),
     ylim=c(0,6e-4),
     xlab="Projected Sea-Level Rise in 2100 [mm]",
     ylab="Probability density function", main="(b) Extended Scenario of Co-CAT (2010)")
# add the zero line
lines(xt,uniform.fit.CA*0,lwd=1,lty=2,col="gray")
#arrows(lb.2100,y.loc.arrow.a,ub.2100,y.loc.arrow.a,col="black",lty="solid",code=3);
abline(v=lb.2100,lwd=1,col="blue",lty=2)
abline(v=ub.2100,lwd=1,col="blue",lty=2)
...
write.table(beta.dist,file="./output/array_uniform.txt",quote=FALSE,col.names=FALSE)


# zoom into the joint distribution of t.star and c.star 
figure(pdf(file="./output/cstart_tstar.pdf"))
