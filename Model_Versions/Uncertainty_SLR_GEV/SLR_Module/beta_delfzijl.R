...
pdf(file="Figures/hindcast.pdf", height = 4.5, width = 6)  # write to pdf, define a pdf file to write to
...
# projections figure
pdf(file="Figures/projections.pdf", height = 4.5, width = 6)  
par(oma = c(0,0,0,0)+0.1, mar = c(4,5,1,1)+0.1)
...
pdf(file="Figures/fitted_beta.pdf",height=10, width=6) 
par(mfcol=c(2,1))
...
write.table(beta.dist,file="./Output/array_beta.txt", quote=FALSE, col.names=TRUE, row.names = FALSE)


# zoom into the joint distribution of t.star and c.star 
figure(pdf(file="Figures/cstar_tstar.pdf"))
...
# Save output
save.image(file = "beta_sl.RData")

# Plot projectiosn with marginal pdf of SLR at 2100
pdf("Figures/projections_constrain.pdf", width = 6.5, height = 4.5)
...
nexamp=100
nrand=sample(N/2,nexamp)  # decrease sampling due to cutting off outliers
high.proj = seq(30000, length(scenario.constrain[,1])) # sample scenarios at high end
...
text(x=2000, y = 1.65, labels = "Hindcast", adj = c(1, 0.5), font = 3, cex = 0.9)
text(x=2030, y = 1.65, labels = "Projections", adj = c(0, 0.5), font = 3, cex = 0.9)
