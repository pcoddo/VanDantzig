...
# Load GEV storm surge parameters:
GEV_param <- read.table("../../../Storm_Surge_Module/Output/param_full.txt", sep = '\t', header = T)
mu <- sample(GEV_param$mu, n_obs, replace = TRUE)
xi <- sample(GEV_param$xi, n_obs, replace = TRUE)
logsigma <- sample(GEV_param$logsigma, n_obs, replace = TRUE)
sigma <- exp(logsigma)

GEV_param <- data.frame(xi, mu, sigma)
rm(mu, xi, logsigma, sigma)

# Combine new parameters into single data frame
Parameters <- data.frame(vanDantzig_param[3:6], beta.dist[1:5], GEV_param)
rm(beta.dist, GEV_param, vanDantzig_param)
